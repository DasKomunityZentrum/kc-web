 ...
 *
 * Created by PhpStorm.
 * Filename: DepartementCountFilter.php
 * User: Tomáš Babický
 * Date: 21.03.2021
 * Time: 14:33
 */
...
class DepartmentCountFilter
{
    ...
    {
        if ($count === 0) {
            return 'Bohužel, ještě žádné Oddělení nemáme';
        } elseif ($count === 1) {
            return 'Dneska už máme jedno Oddělení!';
        } elseif ($count < 5) {
            $parsed = $this->countWordItService->parse($count);

            return 'Dneska už máme ' . $parsed . ' Oddělení';
        } elseif ($count >= 5) {
            $parsed = $this->countWordItService->parse($count);
